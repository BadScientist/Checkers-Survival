...
class sqlHash:
    ...
    def generate_sql_string_hash(self, password):
        """generates a byte string used for storage in the sql databased. Returns byte hash string of password.\n
        password: string
        \tutf-8 password"""
    ...
    def set_salt(self, rounds=12):
        """set the current salt to a randomized one, returns salt as a byte string. Returns salt as a string.\n
        rounds: Int
